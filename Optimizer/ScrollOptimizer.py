        ...
        for iFinger in range(len(myTask[iTrial]["rawTouchTracks"])):
            ...
            for iPoint in range(len(myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"])):
                 ...
                 if iPoint==0 or iPoint==1:
                     ...
                     #accelerate=(V2-V1)/((positionT3-positionT1)/2)
                     accelerate=(V2-V1)
                     AccelerateArray.append(ComputeAccelerate([positionX3,positionY3,positionT3],[positionX2,positionY2,positionT2],[positionX1,positionY1,positionT1]))
                     if iPoint>3:
                         positionX0=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][iPoint-3]["location"][0]/Device_info[0]
                         positionY0=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][iPoint-3]["location"][1]/Device_info[1]
                         positionT0=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][iPoint-3]["timestamp"]
                         ...
                         JerkArray.append(aa)
                         #JerkArray.append(ComputeJerk([positionX3,positionY3,positionT3],[positionX2,positionY2,positionT2],[positionX1,positionY1,positionT1],[positionX0,positionY0,positionT0]))
                     else:
                         Velocity_t=V2
                         Velocity_t_1=V1
                     ...
                     # if iPoint>4:
                     #     #ChracterVector=[accelerate,Velocity_t,JerkArray[-1]]
                     #     #ReferenceVector=[Ave_accelerate,Ave_Velocity,np.mean(JerkArray[-4:-2])]

                     #     ChracterVector=[AccelerateArray[-1],Velocity_t,JerkArray[-1]]
                     #     ReferenceVector=[np.mean(AccelerateArray[-4:-2]),Ave_Velocity,np.mean(JerkArray[-4:-2])]
                 ...
                 # if len(AccelerateArray)>4:
                 #     AveAccelerate=np.mean(AccelerateArray[-4:-2])
                 #     StdAccelerate=np.std(AccelerateArray[-4:-2])
                 #     if (abs(AccelerateArray[-1])<AveAccelerate+1*StdAccelerate):
                 #         #print(aa,AverageJerk,StdJerk)
                 #         LabelFalse(myTask,TestTrial,iFinger,iPoint)

                 # if len(JerkArray)>4:
                 #     AverageJerk=np.mean(JerkArray[-5:-2])
                 #     StdJerk=np.std(JerkArray[-5:-2])
                 #     if (abs(JerkArray[-1])<AverageJerk+1*StdJerk):
                 #         #print(aa,AverageJerk,StdJerk)
                 #         LabelFalse(myTask,TestTrial,iFinger,iPoint)
