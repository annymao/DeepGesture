...
def LabelByHuman_Swipe(myTask,Device_info):
    ...
    for TestTrial in range(len(myTask)):
        ...
        for iFinger in range(len(myTask[iTrial]["rawTouchTracks"])):
            ...
            for iPoint in range(len(myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"])):
                 ...
                 if iPoint==0 or iPoint==1 or iPoint==2:
                     ...
                     ChracterVector=[Point_to_Point0_Theta_Cos,Point_to_Point0_Theta_Sin]
                     ReferenceVector=[Ave_PointToZeroTheta_Cos,Ave_PointToZeroTheta_Sin]
        ...
        for iFinger in range(len(myTask[iTrial]["rawTouchTracks"])):
            ...
            for iPoint in range(len(myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"])):
                if myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][iPoint]['label']==True:
                    #positionX_t_1=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][iPoint]["previousLocation"][0]/Device_info[0]
                    #positionY_t_1=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][iPoint]["previousLocation"][1]/Device_info[1]

                    positionX_t_1=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][0]["location"][0]/Device_info[0]
                    positionY_t_1=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][0]["location"][1]/Device_info[1]

              

                    positionX=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][-TruePoint]["location"][0]/Device_info[0]
                    positionY=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][-TruePoint]["location"][1]/Device_info[1]
            ...
            #Ave_dx=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][-1]["location"][0]-myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][0]["location"][0]
            #Ave_dy=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][-1]["location"][1]-myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][0]["location"][1]

            Ave_dx=positionX*Device_info[0]-myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][0]["location"][0]
            Ave_dy=positionY*Device_info[1]-myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][0]["location"][1]
            ...
            ds=np.sqrt(NewX*NewX+NewY*NewY)
            
            if len(myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"])>1:

                myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][1]["location"][0]=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][0]["location"][0]+NewX
                myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][1]["location"][1]=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][0]["location"][1]+NewY
                dt=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][1]['timestamp']-myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][0]['timestamp']
                # if ds<=40:
                #     print(NewX,NewY,ds,dt)

        LabelAllFalse(myTask[iTrial])

        for iFinger in range(len(myTask[iTrial]["rawTouchTracks"])):
            if len(myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"])>1:
                myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][0]['label']=True
                myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][1]['label']=True
        ...
        #     CandidateData=list()
        #     for iPoint in range(1,len(myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"])):
                 
        #         if myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][iPoint]['label']==True:
        #             positionX_t_1=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][iPoint-1]["location"][0]/Device_info[0]
        #             positionY_t_1=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][iPoint-1]["location"][1]/Device_info[1]
        ...
        #             positionX=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][iPoint]["location"][0]/Device_info[0]
        #             positionY=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][iPoint]["location"][1]/Device_info[1]

                    
        #             PointTheta_t=ComputeTheta(positionX_t_1,positionY_t_1,positionX,positionY)
        #             AllCos.append(np.cos(PointTheta_t))
                    

        #             AllSin.append(np.sin(PointTheta_t))

        #         #myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][iPoint]['label']=False

        # AverageCos=np.mean(AllCos)
        # AverageSin=np.mean(AllSin)

        # NewX=AverageCos*10*tv.SwipeMinimumMovement
        # NewY=AverageSin*10*tv.SwipeMinimumMovement

        
        #LabelAllFalse(myTask[iTrial])

        # for iFinger in range(len(myTask[iTrial]["rawTouchTracks"])):
        #     for iPoint in range(len(myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"])-1,1,-1):
        #         if myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][iPoint]['label']==True:
        #             #print("point",iPoint)
                    
        #             #myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][iPoint]["location"][0]=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][1]["location"][0]+NewX
        #             #myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][iPoint]["location"][1]=myTask[iTrial]["rawTouchTracks"][iFinger]["rawTouches"][1]["location"][1]+NewY
...
def SwipeOptimizer_MyAlgorithm(ValidationData,Device_info,iTrial):
    ...
    if tv.SwipeTask(OptimizedData[iTrial],iTrial)==False:
        print("--my algorithm fail--")
...
def SwipeOptimizer_TestPersonalTouch(ValidationData,Device_info,iTrial,HD,TA,IG,IG_state,AT,JerkT):
    ...
    #print("iOS:",tv.SwipeTask(ValidationData['swipeTask']['trials'][iTrial],iTrial),"vs","MyTouch",tv.SwipeTask(FilteredJson(TaskData)[iTrial],iTrial)) 
